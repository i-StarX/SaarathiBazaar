    ...
    test('02 Create Personal Loan Request', { tag: ["@regression_saarathi"] }, async ({ page }) => {
...
await allure.step("Click Personal : ", async() =>{ await page.mouse.click(221.5, 163.6); 
         await page.waitForTimeout(1500); });

await allure.step("Select Loan Type : 1", async() =>{ await page.mouse.click(46, 223.6); 
for (let i = 1; i < 1; i++) {
